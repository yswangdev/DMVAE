# Scenario s03 -- Splatter: 6 equal clusters across 3 batches, 6,000 cells,
# 20 replicates.
#
# 2,000 genes, de.prob = 0.24, de.facLoc = 0.40, de.facScale = 0.20,
...
path <- paste0(file.path(sim_root, "s03_batch_effect"), "/")
...
process_simulation <- function(i) {
  sim <- splatSimulateGroups(
    params,
    nGenes = 2000,
    group.prob = rep(1 / 6, 6),
    de.prob = rep(0.24, 6),
    de.facLoc = 0.4,
    de.facScale = rep(0.2, 6),
    de.downProb = 0.5,
    verbose = FALSE,
  ...
  sim_meta <- as.numeric(sim@colData@listData$Group)
