"""Run scAce on simulated data, matching the scAce github pipeline.
...
``sim_{idx}.h5``, the interface scAce/reproducibility/Simulation/run_scace.py uses.
Preprocessing uses scAce's own ``data_preprocess`` (default flags: filter ->
normalize_per_cell -> log1p -> scale), exactly as the github run script.
"""
...
parser.add_argument("--rep-start", type=int, default=1, help="Start index for data")
parser.add_argument("--rep-end", type=int, default=10, help="End index for data (exclusive)")
parser.add_argument(
    "--resolution",
    type=float,
    default=2.0,
    help="Leiden resolution for scAce cluster initialisation (scAce default 2). "
    "Higher -> more initial clusters; adaptive merging then reduces k.",
)
parser.add_argument(
    "--tag",
    type=str,
    default="",
    help="Suffix for output filenames, e.g. '__res1.5', so parameter sweeps do not overwrite each other.",
)
parser.add_argument(
    "--input-format",
    choices=("txt", "h5"),
...
def run_one(idx: int):
    ...
    # float so in-place normalize_per_cell can divide
    adata = sc.AnnData(x.astype(np.float32))
    adata.obs["celltype"] = y

    set_seed(42 + idx)

    # ---- scAce preprocessing (github run_scace.py): default data_preprocess ----
    adata = data_preprocess(adata)

    # ---- Run scAce ----
    adata, nmi, ari, K, pred_all, emb_all, _ = run_scace(
        adata, cl_type="celltype", resolution=args.resolution, return_all=True
    )

    # ---- Save results ----
    # With return_all=True, nmi/ari/k are final scalars; emb_all and pred_all are
    # ragged histories that only store as pickled object arrays. Store the final state
    # separately as ordinary arrays.
    pred_final = np.asarray(list(pred_all)[-1])
    while pred_final.ndim > 1:
    ...
    np.savez(
        ...
        ARI_final=np.asarray(ari, dtype=float),
        NMI_final=np.asarray(nmi, dtype=float),
        K_final=np.asarray(K, dtype=np.int32),
        Embedding_final=np.asarray(list(emb_all)[-1], dtype=np.float32),
        Clusters_final=pred_final.astype(np.int32),
