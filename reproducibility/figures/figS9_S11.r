#!/usr/bin/env Rscript
#
# Supplementary Figures S9, S10 and S11 -- the CD4 T1D dataset, at the width the
# main text could not carry:
#   S9   dot plots of the top 10 marker genes per cluster
#   S10  GSEA NES heatmaps, cluster-vs-rest
#   S11  GSEA NES heatmaps, WT-vs-KO
#
...
#
# Usage:
#   Rscript figS9_S11.r                 # both parts
#   PARTS=dot  Rscript figS9_S11.r      # dot plots only (needs the Seurat object)
#   PARTS=gsea Rscript figS9_S11.r      # heatmaps only (CSVs only; runs locally)
#
# ===========================================================================
...
result_path <- Sys.getenv(
  "RESULT_PATH",
  file.path(DIRECTORY, "results", "dmvae", "CD4", "0224",
...
for (k in target_k) {
  ...
  png_path <- file.path(OUT_DIR, paste0("figS9_dotplot_top", top_n_marker_plot,
...
for (sp in specs) {
  for (db in dbs) {
    ...
    figure_tag <- if (sp$tag == "ClusterVsRest") "figS10" else "figS11"
    png_path <- file.path(OUT_DIR, sp$tag,
                          sprintf("%s_GSEA_%s_%s_heatmap_%s.png",
                                  figure_tag, db, sp$tag, sp$ktag))
