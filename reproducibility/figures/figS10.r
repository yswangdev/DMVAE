#!/usr/bin/env Rscript
#
# Supplementary Figure S10 -- p(c = cluster_i | k) panels: the DMVAE soft cluster
...
# Usage:
#   Rscript figS10.r
#   K=6 Rscript figS10.r
#   UMAP_INPUT=/path/data_norm.txt Rscript figS10.r   # embed the raw data instead
#
# Optional:
...
# --- inputs ------------------------------------------------------------------
PCZ <- Sys.getenv("PCZ", "/Volumes/SSD/MCW/Research/Aim 1/Results/p_c_z_best.npy")
DATA_PATH <- Sys.getenv("DATA_PATH",
                        "/Volumes/SSD/MCW/Research/Aim 1/Data/mouse_e/data_norm.txt")
...
LABEL_K <- as.integer(Sys.getenv("LABEL_K", as.character(K)))
...
base <- if (LABEL_K == K) sprintf("pck_umap_k%d", K) else
  sprintf("pck_umap_k%d_labelled_k%d", K, LABEL_K)
