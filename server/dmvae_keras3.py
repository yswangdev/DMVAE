...
class VADE(Model):
    ...
    def train_step(self, data):
        with tf.GradientTape() as tape:
            ...
            p_c_z = tf.math.exp(tf.reduce_sum((tf.math.log(theta_tensor3) - 0.5 * tf.math.log(2 * math.pi * lambda_tensor3) -
                                   tf.math.square(Z - u_tensor3) / (2 * lambda_tensor3)), axis=2)) + 1e-10
            ...
            reconstruction_loss = alpha * original_dim * tf.keras.losses.MeanSquaredError()(data, reconstruction)
            k13 = tf.math.exp(z_log_var_t) / lambda_tensor3

            
            kl_loss = tf.reduce_sum(0.5 * gamma_t * (latent_dim * tf.math.log(math.pi * 2) + tf.math.log(lambda_tensor3) + k13 + tf.math.square(z_mean_t - u_tensor3) / lambda_tensor3), axis=(1, 2, 3)) \
                - 0.5 * tf.reduce_sum(z_log_var + 1, axis=-1) \
                - tf.reduce_sum(tf.reduce_sum(tf.math.log(tf.repeat(tf.expand_dims(self.theta_p, 0), batch_size, axis=0) * p_k) * gamma, axis=-1), axis=1) \
                + tf.reduce_sum(tf.reduce_sum(tf.math.log(gamma) * gamma, axis=-1), axis=1)
...
for i in range(args.start, args.end):
    ...
    for j in range(0, args.m):
        ...
        #np.savetxt(output_datafile + "ae_zmean.txt", ae_zmean)
        ...
        if loss < best_loss:
            ...
            # UMAP visualization for VADE latent space (only for the best iteration)
            reducer = umap.UMAP(n_neighbors=15, min_dist=0.1, metric='euclidean')
    ...
    #np.savetxt(output_datafile + 'umap_vade-k.txt', best_embedding)
    np.savetxt(output_datafile + 'z_mean.txt', best_z_mean)
    np.savetxt(output_datafile + 'vade_loss.txt', best_loss_curve)
    ...
    # Save UMAP visualization for the best VADE latent space
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 14))
    ...
    ax2.legend(*scatter2.legend_elements(), loc="center left", bbox_to_anchor=(1, 0.5), title="Classes")
    plt.tight_layout()
    plt.savefig(output_datafile + "umap_vade_best.png", bbox_inches='tight')
    plt.close(fig)
    plt.clf()
