...
parser.add_argument("--n_centroid", type=int, default=3, help="number of clusters")
parser.add_argument("--epoch", type=int, default=200, help="Number of training epochs")
parser.add_argument("--lr_nn", type=float, default=1e-4, help="Learning rate for neural network")
parser.add_argument("--lr_gmm", type=float, default=1e-4, help="Learning rate for GMM")
parser.add_argument("--ae_lr", type=float, default=5e-6, help="Learning rate for autoencoder pretraining")
parser.add_argument("--ae_epoch", type=int, default=20, help="Epochs for autoencoder pretraining")
...
for i in range(args.start, args.end):
    ...
    for j in range(0, args.m):
        ...
        if last_loss < best_loss:
            ...
            best_acc = accuracy
            best_assign_c = pcz[-1]
            SAE.save(args.output_hyp + "ae_best")

            embedding = reducer.fit_transform(z_mean)
    

        print(f"Finished dataset {i} iteration {j}...")
    ...
    np.savetxt(output_datafile + 'vade_accuracy.txt', best_acc)

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 14))
    scatter1 = ax1.scatter(embedding[:, 0], embedding[:, 1], c=best_assign_c, s=1)
