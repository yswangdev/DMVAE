...
hyperparams = {
    "input_directory": os.path.abspath(args.input_datafile),
...
with open(os.path.join(args.output_hyp, "hyperparameters.json"), "w") as f:
...
class Sampling(Layer):
    ...
    def call(self, inputs):
        ...
        bsz = tf.shape(z_mean)[0]
        epsilon = tf.random.normal(shape=(bsz, latent_dim))
...
class get_gamma(Layer):
    def call(self, inputs):
        # inputs: z with shape (B, latent_dim)
        bsz = tf.shape(inputs)[0]
        temp_Z = tf.tile(tf.expand_dims(inputs, axis=2), [1, 1, n_centroid])          # (B, d, K)
        temp_u_tensor3 = tf.tile(tf.expand_dims(u_p, 0), [bsz, 1, 1])                 # (B, d, K)
        temp_lambda_tensor3 = tf.tile(tf.expand_dims(lambda_p, 0), [bsz, 1, 1])       # (B, d, K)
        temp_theta_tensor3 = tf.ones((bsz, latent_dim, n_centroid)) * tf.reshape(theta_p, (1, 1, n_centroid))

        temp_p_c_z = tf.exp(
            tf.reduce_sum(
                (tf.math.log(temp_theta_tensor3)
                 - 0.5 * tf.math.log(2.0 * np.pi * temp_lambda_tensor3)
                 - tf.square(temp_Z - temp_u_tensor3) / (2.0 * temp_lambda_tensor3)),
                axis=1
            )
        ) + 1e-10  # (B, K)

        gamma = temp_p_c_z / tf.reduce_sum(temp_p_c_z, axis=-1, keepdims=True)        # (B, K)
        return gamma
...
class VADE(Model):
    def __init__(self,encoder, decoder, **kwargs):
        ...
        self.theta_p  = self.add_weight("pi",     shape=(n_centroid,),           initializer="zeros", trainable=True)
        self.u_p      = self.add_weight("u",      shape=(latent_dim, n_centroid),initializer="zeros", trainable=True)
        self.lambda_p = self.add_weight("lambda", shape=(latent_dim, n_centroid),initializer="ones",  trainable=True)
        # init pi uniform
        self.theta_p.assign(tf.fill((n_centroid,), 1.0 / float(n_centroid)))
    ...
    def train_step(self, data):
        ...
        orig_dim_f = tf.constant(float(original_dim), dtype=tf.float32)
        alpha_f = tf.constant(float(alpha), dtype=tf.float32)
        ...
        with tf.GradientTape() as tape:
            ...
            # (B, d, K)
            Z           = tf.tile(tf.expand_dims(z,        2), [1, 1, n_centroid])
            z_mean_t    = tf.tile(tf.expand_dims(z_mean,   2), [1, 1, n_centroid])
            z_log_var_t = tf.tile(tf.expand_dims(z_log_var,2), [1, 1, n_centroid])
    
            u_tensor3      = tf.tile(tf.expand_dims(self.u_p,      0), [bsz, 1, 1])
            lambda_tensor3 = tf.tile(tf.expand_dims(self.lambda_p, 0), [bsz, 1, 1])
            theta_tensor3  = tf.ones((bsz, latent_dim, n_centroid), dtype=tf.float32) * tf.reshape(self.theta_p, (1,1,n_centroid))
    
            # p(c|z)
            p_c_z = tf.exp(tf.reduce_sum(
                tf.math.log(theta_tensor3)
                - 0.5 * tf.math.log(two_pi * lambda_tensor3)
                - tf.square(Z - u_tensor3) / (2.0 * lambda_tensor3),
                axis=1
            )) + 1e-10
            gamma   = p_c_z / tf.reduce_sum(p_c_z, axis=-1, keepdims=True)
            gamma_t = tf.tile(tf.expand_dims(gamma, 1), [1, latent_dim, 1])
    
            # per-sample losses (float32)
            recon_per = alpha_f * orig_dim_f * tf.keras.losses.mean_squared_error(data, reconstruction)
            kl_per = tf.reduce_sum(
                0.5 * gamma_t * (latent_dim * tf.math.log(two_pi)
                                 + tf.math.log(lambda_tensor3)
                                 + tf.exp(z_log_var_t) / lambda_tensor3
                                 + tf.square(z_mean_t - u_tensor3) / lambda_tensor3),
                axis=(1, 2)
            ) - 0.5 * tf.reduce_sum(z_log_var + 1.0, axis=-1) \
              - tf.reduce_sum(tf.math.log(tf.tile(tf.expand_dims(self.theta_p, 0), [bsz, 1])) * gamma, axis=-1) \
              + tf.reduce_sum(tf.math.log(gamma) * gamma, axis=-1)
    
            total_per = recon_per + kl_per
            loss = tf.reduce_mean(total_per)  # scalar for backprop
    
        # --- use trainable_variables (not trainable_weights) ---
        grads = tape.gradient(loss, self.trainable_variables)
        self.optimizer.apply_gradients(zip(grads, self.trainable_variables))
    
        self.total_loss_tracker.update_state(total_per)
        self.reconstruction_loss_tracker.update_state(recon_per)
        self.kl_loss_tracker.update_state(kl_per)
...
def load_pretrain_weights(vade):
    ...
    g.fit(sample)
    vade.u_p.assign(tf.convert_to_tensor(g.means_.T, dtype=tf.float32))
    vade.lambda_p.assign(tf.convert_to_tensor(g.covariances_.T, dtype=tf.float32))
