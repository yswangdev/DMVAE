...
epoch,lr_nn,lr_gmm,decay_n,decay_nn,decay_gmm,alpha = 200,1e-4,1e-4,10,0.8,0.8,1
ispretrain = True
ae_lr = 5e-6
ae_epoch = 20
clip_norm = 5
...
for i in range(args.start, args.end):
    ...
    for j in range(0, args.m):
        ...
        fitting_sae = SAE.fit(X, X, epochs=ae_epoch, batch_size=batch_size, shuffle=True, validation_data=(X, X), verbose=0)
        ...
        if last_loss < best_loss:
            ...
            best_ae_path = os.path.join(output_datafile, "ae_sim_best")
            os.system(f"cp -r {current_sae_path} {best_ae_path}")
